...
def approve(conn: sqlite3.Connection, settings: Settings, ref: int | str, *,
    ...
    if item.status == Status.APPROVED.value:
        return Decision(item, "approve", True, "already approved")
    if item.status not in REVIEWABLE:
        return Decision(item, "approve", False,
                        f"cannot approve from status '{item.status}'")
    ...
    if blocking and not force:
        ...
        return Decision(
            item, "approve", False,
            "guardrails block this item: " + "; ".join(reasons) +
            " -- re-run with --force and a written reason to override",
        )
    if blocking and force and not note.strip():
        return Decision(item, "approve", False,
                        "an override requires a reason (--note): it goes into the audit log")
    ...
    return Decision(item, "approve", True, "override recorded" if blocking else "approved")


def reject(conn: sqlite3.Connection, ref: int | str, *, actor: str, note: str = "") -> Decision:
    item = _load(conn, ref)
    if item.status == Status.PUBLISHED.value:
        return Decision(item, "reject", False, "already published; nothing to reject")
    with db.transaction(conn):
    ...
    return Decision(item, "reject", True, "rejected")
...
def request_revision(conn: sqlite3.Connection, ref: int | str, *, actor: str,
    ...
    if not note.strip():
        return Decision(_load(conn, ref), "revise", False,
                        "a revision request needs a note saying what to change")
    item = _load(conn, ref)
    with db.transaction(conn):
    ...
    return Decision(item, "revise", True, "sent back for revision")
...
def edit(conn: sqlite3.Connection, settings: Settings, ref: int | str, *, actor: str,
    ...
    item = _load(conn, ref)
    if item.status == Status.PUBLISHED.value:
        return Decision(item, "edit", False, "already published; edit upstream instead")
    ...
    return Decision(item, "edit", True, f"saved, now {new_status.value}")
